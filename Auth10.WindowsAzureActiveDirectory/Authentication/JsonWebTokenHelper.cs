...
namespace Auth10.WindowsAzureActiveDirectory.Authentication
{
    ...
    public class JsonWebTokenHelper
    {
        ...
        {
            string issuer = JsonWebTokenHelper.GetFormattedPrincipal(issuerPrincipalId, string.Empty, tenantRealm);
            string audience = JsonWebTokenHelper.GetFormattedPrincipal(audiencePrincipalId, audienceHostName, tenantRealm);
        ...
        public static OAuthAccessToken GetOAuthAccessTokenFromACS(string stsUrl, string assertion, string resource)
        {
            ...
            values.Add(JsonWebTokenHelper.ClaimTypeGrantType, "http://oauth.net/grant_type/jwt/1.0/bearer");
            values.Add(JsonWebTokenHelper.ClaimTypeAssertion, assertion);
            values.Add(JsonWebTokenHelper.ClaimTypeResource, resource);
            ...
            JsonWebToken webToken, X509Certificate2 signingCert)
        {
            return String.Format(
                ...
                JsonWebTokenHelper.GenerateAssertion(webToken, signingCert));
        }
